...
# Base URL for TIGER/Line 2020 place shapefiles
...
for FIPS in "${STATES[@]}"; do
  ...
  if [ "$FIPS" == "01" ]; then
    MODE="-d"  # drop/create for first state
  else
    MODE="-a"  # append for others
  fi

  echo "Importing $SHP_FILE into $SCHEMA_TABLE..."
  shp2pgsql -s $SRID $MODE -g $GEOM_COL "$SHP_FILE" "$SCHEMA_TABLE" | \
  PGPASSWORD="$DB_PASSWORD" psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME
done

echo "✅ All states imported into $SCHEMA_TABLE"
